...
class ScratchRAGModel(nn.Module):
    ...
    def generate_output(self, query, documents):
        ...
        documents = [str(doc) if isinstance(doc, tuple) else doc for doc in documents]
    ...
    def generate_output_with_attentions(self, query, retrieved_texts):
        """
        Generate output based on the retrieved documents and return attentions for heatmap.
        """
        # Combine the retrieved texts into a single context
        context = " ".join(retrieved_texts)
        # Tokenize the input query and context
        combined_input = f"{query} {context}"
        ...
        # Generate output while keeping track of attentions
        with torch.no_grad():
        ...
        # Extract generated tokens and attentions
        generated_ids = outputs.sequences[0]
        generated_output = self.tokenizer_generator.decode(generated_ids, skip_special_tokens=True)
        ...
        return generated_output, attentions, generated_tokens
    ...
    def evaluate(self, top_k_values=[5], writer=None):
      ...
      # Call evaluate_with_posterior_heatmap at the end of evaluate
      # We can choose a specific query to generate the heatmap
      sample_query = self.test_queries[10]
      sample_target_output = self.test_target_outputs[10]
      self.evaluate_with_posterior_heatmap(
          sample_query,
    ...
    def evaluate_with_posterior_heatmap(self, query, target_output, top_k=5, writer=None):
        """
        Evaluate the model by generating output and creating a heatmap to show document posteriors.

        Args:
            query (str): The input query.
            target_output (str): The target output for comparison.
            top_k (int): Number of top documents to retrieve.
            writer (SummaryWriter, optional): TensorBoard writer.

        Returns:
            normalized_matrix (np.array): A 2D array containing document posterior probabilities for each generated token.
        """
        ...
        # Step 3: Generate the output sequence
        with torch.no_grad():
            # Generate output
            generated_ids = self.generator.generate(
                input_ids=input_tokens['input_ids'],
        ...
        generated_output = self.tokenizer_generator.decode(generated_ids[0], skip_special_tokens=True)
        generated_tokens = generated_output.split()

        # Prepare decoder input ids for the generated output
        decoder_input_ids = self.tokenizer_generator(
            generated_output,
            return_tensors='pt',
            add_special_tokens=False
        ).input_ids.to(self.device)

        # Add BOS token at the beginning (since the decoder uses shifted inputs)
        bos_token_id = self.tokenizer_generator.bos_token_id
        decoder_input_ids = torch.cat([torch.tensor([[bos_token_id]], device=self.device), decoder_input_ids], dim=1)

        # Step 4: Forward pass with attentions
        with torch.no_grad():
        ...
        # Step 5: Extract cross-attention tensors
        cross_attentions = outputs.cross_attentions  # List of tensors from each layer
        # Stack all cross-attention tensors: Shape -> (num_layers, num_heads, target_len, source_len)
        attentions = torch.stack(cross_attentions)  # Shape: (num_layers, num_heads, tgt_len, src_len)

        # Sum over layers and average over heads
        attentions = attentions.sum(dim=0)  # Shape: (num_heads, tgt_len, src_len)
        attentions = attentions.mean(dim=0).cpu().numpy()  # Shape: (tgt_len, src_len)

        # Step 6: Map source tokens to documents
        # Tokenize retrieved documents separately to get token counts per document
        doc_token_counts = []
        for doc_text in retrieved_texts:
            doc_tokens = self.tokenizer_generator.tokenize(doc_text)
            doc_token_counts.append(len(doc_tokens))

        # Calculate the starting index of documents in the input sequence
        query_tokens = self.tokenizer_generator.tokenize(query)
        query_length = len(query_tokens)
        doc_start_indices = [query_length + sum(doc_token_counts[:i]) for i in range(len(doc_token_counts))]

        # Map tokens to documents
        token_to_doc = np.zeros(attentions.shape[1], dtype=int)  # Initialize with zeros (query tokens mapped to doc 0)
        for idx, start_idx in enumerate(doc_start_indices):
            end_idx = start_idx + doc_token_counts[idx]
            # Ensure end_idx does not exceed the source length
            end_idx = min(end_idx, attentions.shape[1])
            token_to_doc[start_idx:end_idx] = idx + 1  # Document indices start from 1

        # Now compute attentions over documents
        num_docs = len(retrieved_texts)
        doc_attentions = np.zeros((attentions.shape[0], num_docs))  # (tgt_len, num_docs)
        for i in range(num_docs):
            doc_indices = np.where(token_to_doc == i + 1)[0]
            if len(doc_indices) > 0:
                doc_attentions[:, i] = attentions[:, doc_indices].sum(axis=1)
            else:
                doc_attentions[:, i] = 0.0  # Handle cases where no tokens are mapped

        # Normalize each row (for each generated token) to obtain posterior probabilities over documents
        normalized_matrix = doc_attentions / (doc_attentions.sum(axis=1, keepdims=True) + 1e-12)  # Avoid division by zero

        # Step 7: Plot the heatmap for posterior probabilities
        plt.figure(figsize=(12, 6))
        sns.heatmap(
            normalized_matrix,
            cmap="Blues",
            xticklabels=[f"Doc {i+1}" for i in range(num_docs)],
        ...
        plt.savefig(os.path.join(self.log_dir, f"posterior_heatmap.png"))
        plt.close()
    ...
    def pipeline(self,
        ...
        avg_bleu_score, avg_exact_match, avg_rouge_l_score, avg_recall_at_k = self.evaluate(self.top_k, writer)
